...
final class SwingCarouselUi implements CarouselUi {
    ...
    @Override
    public void start(TwitchService twitchService) {
        ...
        new Thread(() -> handleTokenValidationUi(twitchService)).start();
    ...
    private void handleTokenValidationUi(TwitchService twitchService) {
        final boolean userAuthTokenValid = isUserAuthTokenValid(twitchService);
        if (!userAuthTokenValid) {
            LOGGER.info("userAccessToken is not valid: instantiating AuthTokenValidationPanel");
            authTokenValidationPanel = new AuthTokenValidationPanel(twitchService);
    ...
    /**
     * Checks if the user authentication token is valid by using the given TwitchService object.
     *
     * @param twitchService the TwitchService object to use for checking the user authentication token validity
     *
     * @return true if the user authentication token is valid, false otherwise
     */
    public boolean isUserAuthTokenValid(TwitchService twitchService) {
        final boolean userAccessTokenValid;
        try {
            userAccessTokenValid = twitchService.isUserAccessTokenValid(twitchService.getUserAccessToken());
